...
class InventoryController {
  ...
  isAmmunitionProcurable = (
    ...
    if (ammoType) {
      return (
        this.getAmmunitionCount(playerIdx, ammunitionName) <
          ammoType.maxCount && Players[playerIdx].cash >= ammoType.price
      );
    }
    return false;
  };
    ...
    if (this.isAmmunitionProcurable(playerIdx, ammunitionName)) {
      ...
      if (ammoType) {
        ...
        const increasedCount =
          player.getAmmunitionCount(ammunitionName) + ammoType.packageSize;
        player.setAmmunitionCount(
          ammunitionName,
          Math.min(increasedCount, ammoType.maxCount)
        );
  ...
  isGrenadeProcurable = (playerIdx: number, grenadeName: string): boolean => {
    ...
    if (grenadeType) {
      return (
        this.getGrenadeCount(playerIdx, grenadeName) < grenadeType.maxCount &&
  ...
  buyGrenade = (playerIdx: number, grenadeName: string) => {
    if (this.isGrenadeProcurable(playerIdx, grenadeName)) {
      ...
      if (grenade) {
        const player = Players[playerIdx];
        const increasedCount =
          this.getGrenadeCount(playerIdx, grenadeName) + grenade.packageSize;
        player.setGrenadeCount(
          grenadeName,
          Math.min(increasedCount, grenade.maxCount)
        );
  ...
  isWeaponProcurable = (playerIdx: number, weaponName: string) => {
    ...
    if (weapon) {
      const player = Players[playerIdx];
      return (
        !this.isWeaponInInventory(playerIdx, weaponName) &&
        player.cash >= weapon.price
      );
    }
    return false;
  };
