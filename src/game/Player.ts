...
export class Player {
  ...
  getAmmunitionCount = (ammunitionName: string): number => {
    const ammoCounter = this.ammunition
        .find((ammoCounter) => ammoCounter.ammoName === ammunitionName);
  ...
  setAmmunitionCount = (ammunitionName: string, value: number) => {
    ...
    if(ammoCounter) {
      ...
      this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Ammunition));
    }
  }

  setCash = (value: number) => {
    this.cash = value;
    this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Cash));
  }

  getGrenadeCount = (grenadeName: string): number => {
    const grenadeCounter = this.grenades
        .find((grenadeCounter) => grenadeCounter.ammoName === grenadeName);
  ...
  setGrenadeCount = (grenadeName: string, value: number) => {
    ...
    if(grenadeCounter) {
      ...
      this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Grenades));
    }
  }
  ...
  setLives = (value: number) => {
    this.lives = value;
    this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Lives));
  }

  setScore = (value: number) => {
    this.score = value;
    this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Score));
  }

  isWeaponInInventory = (weaponName: string): boolean => {
    const ownership = this.weapons.find((weaponOwnership) => weaponOwnership.weaponName === weaponName);
  ...
  setWeaponInInventory = (weaponName: string, value: boolean) => {
    ...
    if(ownership) {
      ...
      this.changeListeners.forEach((listener) => listener.call(value, PlayerProperties.Weapons));
    }
  }
  ...
  isRpgSelected = (): boolean => {
    ...
    if((undefined !== selected) && ('ammo' in selected)) {
      ...
      return (('89 mm' === weapon.ammo) || ('60 mm' === weapon.ammo) || ('110 mm' === weapon.ammo));
    }
    return false;
  }
  ...
  selectFirstWeapon = () => {
    ...
    this.changeListeners.forEach((listener) => listener.call(this.selectedWeaponName, PlayerProperties.SelectedWeapon));
  }
  ...
  private getNamesOfWeaponsAndGrenadesInInventory(): string[] {
    const weaponNames = this.weapons.filter((wo) => wo.owned).map((wo) => wo.weaponName);
    const grenadeNames = this.grenades.filter((ac) => ac.count > 0).map((ac) => ac.ammoName);
